...
class ShapeDetector:
    ...
    def __init__(self):
        ...
        self.morph_ksize = PRE_PROCESSAMENTO.get("morph_kernel_size", (3, 3))
        # Filtragem
        ...
        self.solidez_star_max = SOLIDEZ.get("star_max", 0.62)
    ...
    def detect(self, frame):
            ...
            bordas_canny = cv2.morphologyEx(bordas_canny, cv2.MORPH_CLOSE, kernel)
                    ...
                    if hull_indices is not None and len(hull_indices) > 3:
                        try:
                            ...
                            if defects is not None:
                                ...
                                if (5 <= num_defects <= 11):
                                    # ESTRELA
                                    if num_defects == 5 and (self.solidez_star_min < solidez < self.solidez_star_max):
                                        shape_data.update({'label': 'Estrela', 'contour': cnt})
                                        shapes_detectados.append(shape_data)
                                        continue

                                    # CRUZ
                                    elif num_defects == 8 and (self.solidez_cross_min < solidez < self.solidez_cross_max):
